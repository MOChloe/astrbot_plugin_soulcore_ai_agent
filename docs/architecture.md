...
## 角色级思考、Fill Budget 与模型容量

`thinking_complexity` 由 `role_profiles` 唯一持有，六档默认“标准”。MainCore 在每轮开始时按 `profile_id` 读取并冻结不可变策略，上下文准备、工具循环与回复润色共用该轮快照；管理员和快速设置只更新选中的角色，不保留全局或进程级可变档位。每档直接给出最多行动轮次和 Fill Budget：后者限制平时预装的聊天、记忆、背景与当前资料，不是模型规格，也不会要求模型容量达到某个预设数字。

所有承担文字、快速判断、视觉理解或中文润色的模型必须声明服务商公布的真实总上下文容量，缺失时不猜测 128K、256K 或其他默认值，并停止把该模型用于文字请求。SoulCore 没有额外的 30K／128K 保存门槛；每次请求按实际输入文本与图片估算总量，优先使用当前模型，只有实际输入放不下时才裁剪或切换到容量足够的后备模型。只有明确需要固定长度的内部任务才把自己的输出上限计入本轮预算。更大模型不会让平时装入量越过 Fill Budget，更小模型也不会仅因低于某个思考档位预设而被跳过。纯图片生成模型不使用这套文字上下文容量。
...
每个文字模型还可以保存一份高级 JSON 请求参数，并只在该模型实际被选中时合并到 Provider 请求体；切换后备模型会重新读取后备模型自己的配置。`thinking` 等供应商专属嵌套字段会保持 JSON 类型，常用生成参数仍使用单独的受控表单。模型设置不保存“最大输出 Token”：普通 OpenAI 与兼容请求不发送输出上限，交给模型服务按自身规则处理；Anthropic Messages 的 `max_tokens` 是协议必填字段，由运输层按本轮剩余容量在内部生成；少数需要结构化短结果的内部任务可以为单次请求声明上限。旧配置中的三个输出上限字段在读取与下次保存时被丢弃。`model`、消息正文、系统上下文、输出上限和 Prompt 缓存结构不能通过高级 JSON 覆盖；HTTP 适配器当前只接收完整 JSON 响应，因此 `stream` 仅允许显式设为 `false`。高级参数会进入真实请求调试记录，不能用于保存 API Key 或其他秘密。
